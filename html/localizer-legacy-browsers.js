...
function setupTRIGetcRoutineBegin(snapshot) {
  return async function () {
    ...
    setupTRIGetcComponents.forEach( function(thisComponent) {
...
function probe_routineRoutineBegin(snapshot) {
  return async function () {
    ...
    probe_routineComponents.forEach( function(thisComponent) {
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
       });
    return Scheduler.Event.NEXT;
...
function response_routineRoutineBegin(snapshot) {
  return async function () {
    ...
    response_routineComponents.forEach( function(thisComponent) {
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
       });
    return Scheduler.Event.NEXT;
